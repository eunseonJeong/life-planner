...
import { CalculatorData, CalculatorFormData, CalculationResult } from '@/types/calculator'
...
export default function CalculatorPage() {
  ...
  const calculateFinancialPlan = (data: CalculatorData) => {
    ...
    while (currentAmount < targetAmount && months < 600) { // 최대 50년
      currentAmount = currentAmount * (1 + monthlyReturn) + actualMonthlySavings
      months++
  ...
  const handleSaveCalculator = async (data: CalculatorFormData) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // 새로 추가된 계산기를 선택
        if (modalMode === 'create') {
          const newCalculator = response.data.find(item => item.name === data.name)
          if (newCalculator) {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (selectedCalculator?.id === id) {
          setSelectedCalculator(response.data.length > 0 ? response.data[0] : null)
        }
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">연봉별 1억 달성 시점을 자동으로 계산해보세요</p>
        </div>
      ...
      {!isLoading && calculatorList.length === 0 && (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">계산기가 없습니다</h3>
            <p className="text-gray-600 mb-4">새로운 재무 계산기를 추가해보세요!</p>
            <Button onClick={handleOpenCreateModal}>
              <Plus className="mr-2 h-4 w-4" />
              새 계산기 추가
            </Button>
              ...
              <CardTitle className="flex items-center justify-between">
                ...
                {selectedCalculator && (
                  <div className="flex space-x-2">
                    ...
                    <Button 
                      onClick={() => handleDeleteCalculator(selectedCalculator.id!)}
                      variant="outline" 
                      size="sm"
                ...
                {calculatorList.map((calculator) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      ...
                      <p>현재 연봉: {formatCurrency(calculator.currentSalary)}원</p>
                      <p>목표 금액: {formatCurrency(calculator.targetAmount)}원</p>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <div className="space-y-4 max-h-96 overflow-y-auto">
                    {results.yearlyData.map((yearData, index) => (
                      <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                        <div>
                          <h3 className="font-medium">{yearData.year}년 ({yearData.age}세)</h3>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                                         {Object.entries(selectedCalculator.portfolio).map(([key, value]) => (
                       <div key={key} className="flex items-center justify-between">
                         <div className="flex items-center space-x-2">
                           <div className={`w-4 h-4 ${getPortfolioColor(key as keyof CalculatorData['portfolio'])} rounded`}></div>
      ...
      <CalculatorModal
        ...
        onSave={handleSaveCalculator}
                 initialData={modalMode === 'edit' && selectedCalculator ? selectedCalculator : undefined}
        mode={modalMode}
      />
    </div>
  )
